...
func finish_drag():
	...
	var reason
	if card_slot_found:
		var results = global_scripts.validate_card_move(card_being_dragged, card_slot_found)
		valid_move = results[1]
		reason = results[2]
	
	if reason:
		print(reason)
		#$"../Ace/SpeechBubble".text = "[right]" + str(reason) + "[/right]"
	...
	if valid_move == true:
		...
		if reason:
			$"../Deck".draw_card("player") # draw a penalty card
			player_turn_ended.emit() # broke rule, end turn
